...
while (catCount <110) {
  catCount +=25;
  document.getElementById("q5b").innerHTML+="<p>"+"At home, I have "+catCount+" cats."+"</p>";
}
...
var numThrees = function() {
  for (i=0; i<numArray.length; i+=3) {
    document.getElementById("q7").innerHTML+= numArray[i]+", ";
  };
}
numThrees();
...
for (i=0; i<=7; i++){
  ...
  document.getElementById("q8").innerHTML+="<p>"+answer+"</p>";
}
...
function showPrompt(){
  var human = prompt("Rock, Paper, Scissors?");
    ...
    if (humanChoice==="rock" && computer==="scissors") {
      ...
      document.getElementById("winner").innerHTML="It's a tie. Try again."
    }
    else if (humanChoice !== "rock" && humanChoice !== "scissors" && humanChoice !== "paper") {
      document.getElementById("winner").innerHTML="That's not a choice. Please reload and try again."
    }
    else {
...
document.getElementById("q16").innerHTML=actualLargest;
